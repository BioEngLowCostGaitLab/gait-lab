...
{	
	...
	*/
	...
	vector<float> time;
	float t_interval = (float)1.0 / (float)29.9;
	...
	imwrite("C:/Gait-Lab/resources/JSONfiles/plot.jpg", plot_result);
	waitKey();

	FileStorage anglesave("C:/Gait-Lab/resources/JSONfiles/angles.json", FileStorage::WRITE);
	anglesave << "anglesave" << joint_angle;
	anglesave.release();

	vector<float> angletest;
	FileStorage angletake("C:/Gait-Lab/resources/JSONfiles/angles.json", FileStorage::READ);
