...
int calibratemain(vidcamera &vidcam, int camerano, int imgno, int boardheight,int boardwidth,double squareSize,Size imageSize, bool errorcheck, int index)
{
	...
	/*for (int i = 0; i<imagePoints.size(); i++)
	{
		...
		for (int j = 0; j < imagePoints[i].size(); j++)
		{
			...
			cout << objectPoints[i][j] << endl;
		}
	}*/

	double rms;
	rms = calibrateCamera(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs,tvecs, CALIB_FIX_ASPECT_RATIO|CALIB_FIX_PRINCIPAL_POINT);
	...
	cout << "Camera " << index << " has a reprojection error of " << rms << endl;
	cout << endl;
